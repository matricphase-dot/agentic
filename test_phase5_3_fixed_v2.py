# test_phase5_3_fixed_v2.py - Fixed version
import sys
import os
import logging
...
def test_simple_workflow():
    ...
    try:
        ...
        # FIXED: Check if total_duration exists before using it
        if hasattr(result, 'total_duration'):
            print(f"  Duration: {result.total_duration or 0:.2f} seconds")
        else:
            print(f"  Duration: 0.00 seconds (total_duration not set)")
        ...
        if result.overall_status.name == "COMPLETED":
            print("[SUCCESS] Workflow completed successfully")
            return True
        else:
            print("[WARNING] Workflow did not complete")
...
def main():
    """Run all tests"""
    print("AGENTIC WORKFLOW ENGINE - COMPREHENSIVE TEST v2")
    print("=" * 70)

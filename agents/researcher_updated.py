    ...
    def test(self) -> bool:
        ...
        for task in test_tasks:
            try:
                ...
                if result["success"]:
                    print(f"  ✅ {task[:30]}... - Success")
                    passed += 1
                else:
                    print(f"  ❌ {task[:30]}... - Failed: {result.get('error')}")
            except Exception as e:
                print(f"  ❌ {task[:30]}... - Error: {e}")
        
        success = passed >= 2
        print(f"\n✅ Researcher Agent test {'PASSED' if success else 'FAILED'} ({passed}/{len(test_tasks)} passed)")
        return success
...
if __name__ == "__main__":
    ...
    # Test 1: List tools
    tools = researcher.list_tools()
    print(f"\n1. Tools available: {tools}")
    
    # Test 2: Execute task
    print("\n2. Executing research task...")
    result = researcher.execute_task("Check package information")
    ...
    print("\n3. Running full test...")
    if researcher.test():
        print("\n✅ Researcher Agent is ready!")
    else:
        print("\n⚠ Researcher Agent needs attention")